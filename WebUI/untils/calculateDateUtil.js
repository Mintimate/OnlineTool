export function DateDifference(faultDate, completeTime) {
    ...
    const stime = typeof faultDate === 'string'
        ? new Date(faultDate).getTime()
        : faultDate?.getTime();

    const etime = typeof completeTime === 'string'
        ? new Date(completeTime).getTime()
...
export function addDate(startTime,days){
    const stime = typeof startTime === 'string'
